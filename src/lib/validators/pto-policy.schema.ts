...
const postingScheduleSchema = z.object({
  ...
  posting2ServiceMonthBasis: ServiceMonthBasisEnum.nullable().optional(),
  posting2AnchorDate:       z.string().regex(/^\d{4}-\d{2}-\d{2}$/).nullable().optional().transform(v => v ? new Date(v + "T12:00:00Z") : null),
  posting2StartsOnYear:     z.number().int().min(1).max(99).nullable().optional(),
  posting2BasedOnMonths:    z.boolean().default(false),
